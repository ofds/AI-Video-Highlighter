...
class VideoProcessor:
    ...
    def generate_highlights_data(self) -> Optional[List[Dict[str, str]]]:
        """
        Runs the analysis part of the pipeline: audio extraction, transcription,
        and LLM analysis. Returns a list of structured highlight data.
        """
        ...
        if self.transcript_path.is_file():
            ...
            if not self._extract_audio():
                return None # Stop if audio extraction fails
        ...
        # --- Step 2: Get and Parse Highlights Text ---
        if full_transcript_text:
            ...
            if highlights_text:
                return self._parse_highlights_to_structured_data(highlights_text)
    ...
    def create_highlight_video(self, time_segments: List[Tuple[str, str]]):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for i, (start, end) in enumerate(time_segments):
                ...
                try:
                    # Using PIPE for stdout and stderr to keep console clean
                    ...
                    return # Abort if any clip fails
        ...
        self._cleanup() # Clean up temp audio file after video is made
    ...
    def _transcribe_audio(self) -> Optional[List[Dict[str, Any]]]:
        ...
        try:
            ...
            result = model.transcribe(str(self.temp_audio_path), fp16=False) # fp16=False for better compatibility
    ...
    def _parse_highlights_to_structured_data(self, highlights_text: str) -> List[Dict[str, str]]:
        ...
        try:
            ...
            # Split by the numbered entries. This looks for a number, a dot, and then whitespace.
